...
def draw_walls_and_rects(ax, obstacles_data):
...
# エージェントクラス
class Customer:
    def __init__(self, start_pos, waypoints, goal, anim_interval):
    ...
    def update(self):
        ...
        if dist < 0.2:
            ...
            if is_final_goal:
                ...
                self.target_index += 1 # 先に次の目標インデックスに進める
                
                if wait_seconds > 0:
                    self.is_waiting = True
                    self.wait_timer = int(wait_seconds * 1000 / self.anim_interval)
                    return True # 待機する場合は、ここでフレームの処理を終了
                
                # 待ち時間が0の場合は1フレームも止まらない
                current_target_info = self.targets[self.target_index]
                target_pos = current_target_info["pos"]
                vec = target_pos - self.pos
                dist = np.linalg.norm(vec)
...
if __name__ == '__main__':
    ...
    ax.set_title("Cafeteria Customer Flow Model")

    # 静的レイアウトの描画
    draw_shelves(ax, CANDY_SHELVES_BASE, SHELF_COLOR, SHELF_WIDTH, SHELF_HEIGHT)
    draw_walls_and_rects(ax, WALLS_LINE + SPACES_RECT)
    ...
    customer = Customer(INITIAL_POSITION, waypoints, GOAL_POS, ANIMATION_INTERVAL)
